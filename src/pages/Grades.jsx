...
function Grades({ user }) {
    ...
    try {
      ...
      data.students.forEach(function (student) {
        ...
        initialMarks[student.studentId] = {
          assignmentMarks:
            existingGrade?.assignmentMarks ?? "",
          quizMarks:
            existingGrade?.quizMarks ?? "",
          midtermMarks:
            existingGrade?.midtermMarks ?? "",
          finalMarks:
            existingGrade?.finalMarks ?? "",
          practicalMarks:
            existingGrade?.practicalMarks ?? "",
          participationMarks:
            existingGrade?.participationMarks ?? "",
  ...
  async function saveGrade(studentId, status) {
    ...
    try {
      ...
      for (const field in markLimits) {
        const value = Number(
          studentMarks[field] || 0
        );

        if (
          value < 0 ||
          value > markLimits[field]
        ) {
          setError(
            `${field.replace(
              "Marks",
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/grades`,
        {
          ...
          body: JSON.stringify({
            ...
            quizMarks: Number(
              studentMarks.quizMarks || 0
            ),
            midtermMarks: Number(
              studentMarks.midtermMarks || 0
            ),
            finalMarks: Number(
              studentMarks.finalMarks || 0
            ),
      ...
      setGrades(function (currentGrades) {
        const withoutOld = currentGrades.filter(
          function (grade) {
  ...
  function generateStudentGradePDF(grade) {
    ...
    doc.text("EDUCORE", 105, 20, {
      align: "center"
    });

    doc.setFontSize(14);
    doc.setFont("helvetica", "normal");
    doc.text("Academic Grade Report", 105, 29, {
      align: "center"
    });
    ...
    doc.text(
      `Course Code: ${
        grade.courseCode || grade.courseId
      }`,
      20,
      66
    );
    ...
    autoTable(doc, {
      ...
      body: [
        [
          "Assignment",
          `${grade.assignmentMarks || 0} / 10`
        ],
        ["Quiz", `${grade.quizMarks || 0} / 10`],
        [
          "Midterm",
          `${grade.midtermMarks || 0} / 25`
        ],
        ["Final", `${grade.finalMarks || 0} / 35`],
        [
          "Practical",
          `${grade.practicalMarks || 0} / 10`
        ],
        [
          "Participation",
          `${grade.participationMarks || 0} / 10`
        ],
        [
          "Total",
          `${grade.totalMarks || 0} / 100`
        ]
      ],
      theme: "grid",
      headStyles: {
    ...
    const safeStudentName = studentName.replace(
      /[^a-z0-9]/gi,
      "_"
    );

    doc.save(
      `EduCore-${safeStudentName}-Grade-Report.pdf`
    );
  }

  function generateClassGradePDF(
    classItem,
    classGrades
  ) {
    ...
    doc.setFontSize(22);
    doc.setFont("helvetica", "bold");
    doc.text("EDUCORE", 148, 18, {
      align: "center"
    });

    doc.setFontSize(14);
    doc.setFont("helvetica", "normal");
    doc.text("Class Grade Report", 148, 27, {
      align: "center"
    });
    ...
    const courseCode =
      classItem.courseCode || classItem.courseId;
  ...
  return (
    <main className="mx-auto min-h-screen max-w-7xl bg-app-background px-4 py-8 transition-colors duration-200 sm:px-6">
      <section className="rounded-3xl border border-green-200/70 bg-gradient-to-br from-green-700 via-green-600 to-emerald-500 p-8 text-left text-white shadow-xl shadow-green-900/10 dark:border-green-800">
      ...
      {error && (
        <p className="mt-6 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-red-700 dark:border-red-900 dark:bg-red-950/50 dark:text-red-300">
          {error}
        </p>
      )}

      {message && (
        <p className="mt-6 rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-green-700 dark:border-green-900 dark:bg-green-950/50 dark:text-green-300">
          {message}
        </p>
      )}

      {isFaculty && (
        <section className="mt-8 rounded-2xl border border-app-border bg-app-surface/90 p-6 shadow-sm backdrop-blur">
          <div>
            ...
            <p className="mt-1 text-sm text-app-text-muted">
              Select one of your classes to enter student
              grades.
            </p>
          </div>
          ...
          <select
            ...
            className="mt-5 w-full rounded-xl border border-app-border bg-app-surface-soft px-4 py-3 text-sm text-app-text focus:border-green-500 focus:ring-2 focus:ring-green-100 dark:focus:ring-green-900/50"
          >
      ...
      {isFaculty && selectedClass && (
        <section className="mt-8 rounded-2xl border border-app-border bg-app-surface/90 p-6 shadow-sm backdrop-blur">
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-app-text-muted">
              Enter marks out of 100. Grades and GPA are
          ...
          {studentsLoading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[1250px] text-left">
                <thead>
                  <tr className="border-b border-app-border text-sm text-app-text-muted">
                  ...
                  {students.map(function (student) {
                    ...
                    return (
                      <tr
                        ...
                        className="border-b border-app-border transition hover:bg-green-50/40 dark:hover:bg-green-950/20"
                      >
                          ...
                          return (
                            ...
                            >
                              <input
                                ...
                                className="w-20 rounded-lg border border-app-border bg-app-surface-soft px-2 py-2 text-sm text-app-text focus:border-green-500 focus:ring-2 focus:ring-green-100 dark:focus:ring-green-900/50"
                              />
                            </td>
                          );
                        })}

                        <td className="px-3 py-4 font-bold text-app-text">
                          {preview.total}
                        </td>
                        ...
                        <td className="px-3 py-4 font-bold text-green-700 dark:text-green-400">
                        ...
                        <td className="px-3 py-4">
                          <div className="flex gap-2">
                            <button
                              ...
                              className="rounded-lg border border-green-200 px-3 py-2 text-xs font-semibold text-green-800 transition hover:bg-green-50 disabled:opacity-50 dark:border-green-800 dark:text-green-300 dark:hover:bg-green-950/50"
                            >
                              Draft
                            </button>
      ...
      {(isAdmin || isStudent) && (
        <section className="mt-8 rounded-2xl border border-app-border bg-app-surface/90 p-6 shadow-sm backdrop-blur">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <input
                ...
                className="rounded-xl border border-app-border bg-app-surface-soft px-4 py-3 text-sm text-app-text placeholder:text-app-text-muted focus:border-green-500 focus:ring-2 focus:ring-green-100 dark:focus:ring-green-900/50"
              />
              ...
              <select
                ...
                className="rounded-xl border border-app-border bg-app-surface-soft px-4 py-3 text-sm text-app-text focus:border-green-500 focus:ring-2 focus:ring-green-100 dark:focus:ring-green-900/50"
              >
          ...
          {loading ? (
            ...
            <div className="mt-6 rounded-xl border border-dashed border-app-border py-12 text-center">
                  ...
                  {filteredGrades.map(function (grade) {
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-4 font-bold text-green-700 dark:text-green-400">
                          {Number(
                            grade.gpaPoints || 0
                          ).toFixed(1)}
                        </td>
                        ...
                        <td className="px-4 py-4">
                          <button
                            ...
                            className="rounded-lg border border-green-200 px-3 py-2 text-xs font-semibold text-green-800 transition hover:bg-green-50 dark:border-green-800 dark:text-green-300 dark:hover:bg-green-950/50"
                          >
                            PDF
                          </button>
                        </td>
